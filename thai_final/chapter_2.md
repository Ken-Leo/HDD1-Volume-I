...
ในบทนี้จะอธิบายถึงความสำคัญของระบบการบันทึกข้อมูล โดยจะเน้นไปที่อุปกรณ์ฮาร์ดดิสก์ไดรฟ์ (hard disk drive) จากนันจะกล่าวถึงประวัติความเป็นมาของการบันทึกระบบแม่เหล็ก (magnetic recording) พร้อมทั้งอธิบายพื้นฐานต่างๆ เกียวกับการบันทึกระบบแม่เหล็ก เช่น โครงสร้างการจัดเก็บ ข้อมูลในฮาร์ดดิสก์ไดรฟ์, ลูปฮิสเทอรีซิส (Hysteresis loop), และซูเปอร์พาราแมกเนติก (superparamagnetic) เป็นต้น และสุดท้ายจะกล่าวถึงแนวโน้มของเทคโนโลยีการบันทึกข้อมูลต่างๆ ที่จะนำมาใช้ ในฮาร์ดดิสก์ไดรฟ์ในอนาคต เพื่อรองรับความต้องการในการจัดเก็บข้อมูลที่มากขึ้นเรือยๆ

## 1.1บทนำ

ปัจจุบันนี้เป็นยุคของข้อมูลข่าวสาร ดังนั้น จึงมีความต้องการระบบการบันทึกข้อมูลที่มีประสิทธิภาพ สูง, ราคาถูก, และมีความน่าเชื่อถือ เพื่อ ทำหน้าที่ในการจัดเก็บ ข้อมูลที่มือยู่จำนวนมาก อุปกรณ์ สำหรับการบันทึกข้อมูลที่ใช้กันทั่วไปในปัจจุบัน เช่น แผ่นบันทึกแม่เหล็ก (magnetic floppy disk), แถบบันทึกแม่เหล็ก (magnetic tape), ฮาร์ดดิสก์ไดรฟ์, แผ่นซีดี (CD: compact disc), และแผ่นดีวีดี (DVD: digital versatile disc) เป็นต้น ซึ่งมักจะพบการใช้งานอุปกรณ์ เหล่านี้อย่างเห็นได้ชัดภายใน

![](images/chapter_2/955c5bd28a4eb4b2b59534b64846379c60592fb0ca0dc0dcf28d3b45d9291ccc.jpg)  
รูปที่ 11: โครงสร้างลำดับั้นของการบันทึกข้อมูล [1]
...
การบันทึกระบบแม่เหล็กเป็นเทคโนโลยีที่ใช้ในการจัดเก็บข้อมูลในฮาร์ดดิสก์ไดรฟ์ ในหัวข้อนี้จะกล่าว ถึงหลักการทำงานทั่วไปของฮาร์ดดิสก์ไดรฟ์ พร้อมทั้งแสดงให้เห็นว่า ฮาร์ดดิสก์ไดรฟ์สามารถที่จะถูก จำลองให้อยู่ในรูปของระบบสื่อสารดิจิทัล (digital communication system) ได้เพื่อที่จะได้สามารถ ประยุกต์ใช้ทฤษฎีต่างๆ ทางด้านระบบสื่อสารมาใช้ในการอธิบายหลักการทำงานของระบบการประมวล

## 1.4. การบันทึกระบบแม่เหล็ก

![](images/chapter_2/4896605e867f4812796ffffa01050069558914aabb9d25e4bd22e720e31cc2a7.jpg)  
...
จากรูปที่1.5 ลำดับข้อมูล (data sequence) $\{ x _ { k } \}$ จะ ถูก เข้ารหัส ข้อมูลด้วยวงจรเข้ารหัส (encoder) เพื่อช่วย เพิ่มประสิทธิ ภาพในการบันทึก และ การถอดรหัส ข้อมูล จากนั้นลำดับข้อมูลที่ถูกเข้า รหัส $\{ a _ { k } \}$ ก็จะถูกส่งไปยังตัวขับการเขียน (write driver) เพื่อแปลงข้อมูลไบนารีให้เป็นกระแสไฟฟ้า สำหรับป้อนหัวเขียน (พrite head) เพื่อทำการเขียนข้อมูล เข้าไปในสื่อบันทึก จากนั้นเมื่อต้องการ นำข้อมูลออกมาใช้งาน หัวอ่าน (read head) ก็จะ ทำการอ่านข้อมูลจากสื่อบันทึก โดยจะได้ออกมา ในรูปของสัญญาณทางไฟฟ้าที่เรียกกันว่า "สัญญาณ read-back" ซึ่งก็จะ ถูกส่งผ่านเข้าไปในบล็อก ช่องสัญญาณอ่าน (read chaทnel) ซึ่งประกอบไปด้วยอุปกรณ์อิเล็กทรอนิกส์ต่างๆ ที่ทำหน้าที่ในการ ประมวลผลสัญญาณ จากนั้นก็จะ ส่งผลลัพธ์ ที่ได้ไปยังวงจรตรวจหา (dะtector) เพื่อ ตัดสินใจว่าข้อมูล $\{ a _ { k } \}$ ที่ส่งมาคืออะไรและวงจรถอดรหัส (decoder) ก็จะทำการถอดรหัสข้อมูล $a _ { k }$ เพื่อให้ได้ออกมา เป็นค่าประมาณ ของ $x _ { k }$ ซึ่งจะใช้สัญลักษณ์ว่า $\hat { x } _ { k }$ ดังนั้น ถ้า $\hat { x } _ { k } \neq x _ { k }$ ก็แสดงว่ามีข้อผิดพลาด (error) เกิดขึ้นในระบบ สำหรับรายละเอียดของแบบจำลองระบบสื่อสารของฮาร์ดดิสก์ไดรฟ์ สามารถ ศึกษาเพิ่มเติมได้ในบทที่ 6

![](images/chapter_2/49e7f2927ec3aa6516a1a4aeddc4d53c8bad3fbddacd4e16fd7ed07a2081ed22.jpg)  
...
บันทึกมีสภาพความเป็นแม่เหล็กอิ่มตัวแบบลบ (negative saturation) หรือมีสภาพความเป็นแม่เหล็ก   
อิ่มตัวในทิศทางตรงกันข้าม หลังจากนั้นถ้าทำการเพิ่มค่า H ก็จะทำให้ค่า M วิ่งไปที่จุด , จุด , ญ   
...
$H _ { c }$ ของสื่อ บันทึก จะต้องประนี้ประนอมระหว่างความสามารถในการบันทึกข้อมูล และการป้องกัน การรบกวนกันของสนามแม่เหล็กจากแทร็กข้างเคียง ตารางที่1.1 แสดงข้อดีและข้อเสียของการเลือก ใช้สื่อบันทึกที่มีค่า $H _ { c }$ มากและน้อย
...
เมื่อ M คือจำนวนเต็มที่มีค่ามาก เช่น $M = 6 0$ เป็นต้น สมการ (1.4) จะ เป็นตัวบอกขีดจำกัด ทั่วไปว่า "ขีดจำกัดซูเปอร์พาราแมกเนติก (superparamagnetic limit)" [1, 5, 11, 15] นั่นคือ ถ้า ค่า M ในสมการ (1.4) น้อยเกินไป เช่น M = 2 (แสดงว่า $E _ { P }$ มีค่าไม่ต่างจากค่า $E _ { T }$ มาก) โอกาสที่ข้อมูลบิต 0 จะ เกิดการเปลี่ยนแปลงทิศทางสภาพความเป็นแม่เหล็กไปเป็นข้อมูลบิต 1 (หรือ ข้อมูลบิด 1 เปลี่ยนไปเป็นบิดต 0) ก็มีความเป็นไปได้สูง ปรากฎการณ์นี้จะเรียกกันว่า "ผลกระทบของ ซูเปอร์พาราแมกเนติก (superparamagnetic effect)" ึ่งเป็นข้อจำกัดของเทคโนโลยีการบันทึก ข้อมูล ที่ทำให้ความจุข้อมูล ของฮาร์ดดิสก์ไดรฟ์ไม่สามารถ เพิ่มขึ้นไปได้มากกว่าระดับที่ ก่อให้เกิดผลกระทบ ของซูเปอร์พาราแมกเนติก

![](images/chapter_2/54379bb904d3b1d17bfae881adbb93a02963c0c7d543ce25ea521e5b694fc4fc.jpg)  
รูปที่ 1.9: แนวโน้มของเทคโนโลยีที่ใช้ในฮาร์ดดิสก์ไดรฟ์ [16]
...
![](images/chapter_2/f65935bec634554a8017527709c8ae88388fbf7e24bc3a1ea551012fd38dacd6.jpg)  
...
1) สื่อบันทึก สื่อบันทึกที่ใช้กับระบบการบันทึกแบบแนวตั้งจะมี 2 ชึ้น (โayer) ดังแสดงในรูปที่ 1.10 โดยที่ ั้นแรกจะเป็นชั้นแม่เหล็กที่ใช้เก็บข้อมูลบิต 0 และบิด 1 ส่วนชั้นที่สองจะมีชื่อเรียก ว่า "soft underlayer (SบL)" ซึ่งจะทำหน้าที่ในการนำฟลักซ์แม่เหล็กจากชั้นแม่เหล็กให้กลับ ไปสู่หัวแม่เหล็ก ในปัจจุบันนี้นักวิจัยได้พัฒนาคุณภาพของ รUL ให้ดีขึ้นเรื่อยๆ เพื่อรองรับการ ทำงานของระบบการบันทึกแบบแนวตั้งที่ความจุข้อมูลสูง [1, 11, 12]

2) หัวแม่เหล็ก หัวเขียนที่ใช้ในระบบการบันทึกแบบแนวตั้งจะ เกี่ยวข้องกับการออกแบบโพรบ (probe deรigท) เพื่อทำให้สามารถเขียนแทร็กแคบๆ ที่ตั้งฉากกับระนาบของจานบันทึกได้ นอก จากนี้หัวแม่เหล็ก ที่ใช้ในระบบการบันทึก แบบแนวนอนสามารถที่จะนำมาใช้งานในระบบการ บันทึกแบบแนวตั้งได้ [20]
...
## 1.5.3 เทคโนโลยีการบันทึกระบบแม่เหล็กที่ใช้ความร้อนเข้าช่วย

เทคโนโลยีการบันทึกระบบแม่เหล็กที่ใช้ความร้อนเข้าช่วย (HAMR: heat-assisted magnetic recording) [23, 24, 25] เป็นเทคโนโลยีหนึ่งที่สามารถช่วยเพิ่มความหนาแน่นเชิงพื้นที่ให้มากขึ้นได้สูงถึง 400 Gb/in.2 [25] เทคโนโลยี HAMR หรือท่รู้จักกันว่า "การบันทึกแบบไฮบริด (hybrid recording)" ซึ่งมีแบบจำลองการทำงาน ตามรูปที่ 1.11 โดยมีหลักการทำงานบนพื้นฐานของสิ่งต่อไปนี้
...
การอ่านและการเขียนข้อมูลในฮาร์ดติสก์ไดรฟ์จะอาศัยหลักการของ การเปลี่ยนแปลงระดับสภาพ ความเป็นแม่เหล็กของสื่อบันทึก การบันทึกแบบแนวนอนเป็นเทคโนโลยีที่ใช้ในการบันทึกข้อมูลของ ฮาร์ดดิสก์ไดรฟ์ในปัจจุบัน อย่างไรก็ตาม ความจุข้อมูลของฮาร์ดดิสก์ไดรฟที่ใช้เทคโนโลยีการบันทึก แบบแนวนอนนี้ไกล้จะ ถึงขีดจำกัดซูเปอร์พาราแมกเนติก ซึ่งจะทำให้ไม่สามารถเพิ่มความจุข้อมูลให้ มากกว่าขีดจำกัดนี้ได้ ดังนั้นในการจะ เพิ่มความจุข้อมูลให้มากกว่าความสามารถของเทคโนโลยีการ บันทึกแบบแนวนอน จึงจำเป็นต้องใช้เทคโนโลยีการบันทึกข้อมูลแบบใหม่ เช่น เทคโนโลยีการบันทึก แบบแนวตั้ง, เทคโนโลยีการบันทึกระบบแม่เหล็กที่ใช้ความร้อนเข้าช่วย, เทคโนโลยีหน่วยเก็บโพรบ, และเทคโนโลยีฮอโลกราฟี เป็นต้น ซึ่งในปัจจุบันนี้ฮาร์ดดิสก์ไดรฟ์ที่ใช้เทคโนโลยีการบันทึกแบบแนว ตังก็เริมที่จะมีวางจำหน่ายในท้องตลาดแล้ว
...
# พื้นฐานทางคณิตศาสตร์สำหรับฮาร์ดติสก์ ไดรฟ์

ในบทนี้จะกล่าวถึงทฤษฎีพื้นฐานต่างๆ ทางคณิตศาสตร์ที่เกี่ยวข้องกับระบบการประมวลผลสัญญาณ ของฮาร์ดดิสก์ไดรฟ์ โดยจะ เริ่มจากการจำแนก ประเภทของ สัญญาณ จากนั้นก็ จะ อธิบายถึงลักษณะ ของสัญญาณดิจิทัล, คอนโวลูชัน, ระบบ, และสหสัมพันธ์ เป็นต้น เพื่อเตรียมความพร้อมให้กับผู้อ่าน ได้เข้าใจถึงพื้นฐานทางคณิตศาสตร์ต่างๆ ที่จำเป็นสำหรับการวิเคราะห์ระบบการประมวลผลสัญญาณ ของฮาร์ดดิสก์ไดรฟ์ ก่อนที่จะศึกษาในบทต่อๆ ไป สำหรับผู้สนใจที่ต้องการศึกษารายละเอียดเพิ่มเติม ของเนื้อหาในบทนี้ สามารถค้นคว้าได้จาก [29, 32, 33, 34]

## 2.1 ประเภทของสัญญาณ

สัญญาณ (sigกal) เป็นฟังก์ชัน (function) ที่ใช้แสดงปริมาณทางกายภาพ ในทางคณิตศาสตร์สัญญาณ สามารถที่จะ ถูกแทนด้วยฟังก์ชันของตัวแปรอิสระ เช่น สัญญาณ f(t) หมายถึง สัญญาณ f ที่เป็น ฟังก์ชันของตัวแปรอิสระ t โดยทั่วไป สัญญาณสามารถที่จะถูกจำแนกออกเป็น 4 ประเภทใหญ่ๆ [29] ตามคุณลักษณะของสัญญาณ ดังนี้
...
(b)  
รูปที่ 2.1: ตัวอย่าง (a) สัญญาณที่ต่อเนื่องทางเวลา และ (b) สัญญาณที่ไม่ต่อเนื่องทางเวลา

## 2.1.1 สัญญาณที่ต่อเนื่องทางเวลาและสัญญาณที่ไม่ต่อเนื่องทางเวลา

สัญญาณที่ต่อเนื่องทางเวลา (continuous-time signal) คือสัญญาณที่มีการเปลี่ยนแปลงอย่างต่อเนื่อง ทางเวลา โดยทั่วไป จะใช้สัญลักษณ์ $x ( t )$ แทนสัญญาณที่ต่อเนืองทางเวลา x เมื่อ t คือตัวแปร อิสระที่ต่อเนื่องทางเวลา (นั้นคือ ตัวแปร t สามารถมีค่าเป็นจำนวนจริงใดๆ) ดังนั้น ค่าของสัญญาณที่ ต่อเนื่องทางเวลาจึงเป็นสมาชิกของเซตจำนวนจริงหรือเซตไม่จำกัด (inรทite set) ตัวอย่างของสัญญาณ ที่ต่อเนื่องทางเวลา เช่น เสียงพูด และคลื่นวิทยุ เป็นต้น บ่อยครั้งสัญญาณที่ต่อเนื่องทางเวลาจะถูก เรียกว่า "สัญญานแอนะล็อก (analog signal)"

สัญญาณที่ไม่ต่อเนื่องทางเวลา (discrete-time signal) คือ สัญญาณที่ไม่มีการเปลี่ยนแปลงอย่าง ต่อเนื่องทางเวลา ในหนังสือเล่มนี้จะใช้สัญลักษณ์ x[k] หรือ $x _ { k }$ แทนสัญญาณที่ไม่ต่อเนื่องทาง เวลา x เมือ k คือ ตัวแปรอิสระที่ไม่ต่อเนืองทางเวลา (นันคือ ตัวแปร k จะมีค่าจำกัด) ดังนัน ค่า ของสัญญาณที่ไม่ต่อเนื่องทางเวลาจึงเป็นสมาชิกของเซตจำกัด (finite set) ตัวอย่างของสัญญาณ ที ไม่ต่อเนื่องทางเวลา เช่น ดัชนีตลาดหุ้นไทย, จำนวนฮาร์ดดิสก์ไดรฟ์ที่ผลิตได้, และจำนวนประชากร ของกรุงเทพ เป็นต้น รูปที่ 2.1 แสดงตัวอย่างรูปร่างของสัญญาณที่ต่อเนื่องทางเวลาและสัญญาณที่ไม่ ต่อเนืองทางเวลา

นอกจากนี้สัญญาณที่ไม่ต่อเนื่องทางเวลา $x _ { k }$ อาจจะได้มาจากการชักตัวอย่าง (sampling) สัญญาณ ที่ต่อเนื่องทางเวลา x(t) ที่ทุกๆ ช่วงเวลา $t = k T _ { s }$ เมื่อ k เป็นเลขจำนวนเต็ม และ $T _ { s }$ คือ คาบการ

## 2.1. ประเภทของสัญญาณ

ชักตัวอย่าง (sampling period) โดย $f _ { s } = 1 / T _ { s }$ คือ ความถี่ การชักตัวอย่าง (sampling frequency) เพราะฉะนั้น สัญญาณที่ไม่ต่อเนื่องทางเวลาที่ได้จากการชักตัวอย่างสามารถเขียนเป็นสมการทาง คณิตศาสตร์ได้ดังนี้ (ดูรูปที่ 2.1(b))

$$
x [ k ] = x _ { k } = \left. x ( t ) \right| _ { t = k T _ { s } } = x ( k T _ { s } )\tag{2.1}
$$

นั่นคือ ข้อมูล $x _ { k }$ แต่ละตัวจะอยู่ห่างกัน $T _ { s }$ หน่วย

หมายเหตุ จากสมการ (2.1) จะได้ว่า สัญญาณที่ไม่ต่อเนื่องทางเวลา x[k] สามารถใช้เป็นตัวแทนของ สัญญาณที่ต่อเนืองทางเวลา $x ( t )$ ได้อย่างสมบูรณ์ ถ้าความถี่การชักตัวอย่างสอดคล้องกับทฤษฎีบท การชักตัวอย่าง (sampling theorem) ซึ่งจะอธิบายต่อไปในหัวข้อที่ 5.5

## 2.1.2 สัญญาณเป็นคาบและสัญญาณไม่เป็นคาบ

สัญญาณเป็นคาบ (periodic signal) คือ สัญญาณที่มีรูปร่างซ้ำตัวมันเองทุกๆ ช่วงเวลาที่กำหนด ซึ่ง ช่วงเวลานี้จะ เรียกว่า คาบ (period) ของสัญญาณ ดังนั้น สัญญาณเป็นคาบที่ต่อเนืองทางเวลา (periodic contiทนous-time signal) คือ สัญญาณที่ต่อเนื่องทางเวลาที่สามารถหาคาบเวลา T (ที่เป็นค่า บวก)ที่ทำให้

$$
x ( t ) = x ( t + m T )\tag{2.2}
$$

สำหรับทุกค่า t เมื่อ m เป็นเลขจำนวนเต็ม รูปที่ 2.2(a) แสดงตัวอย่างของสัญญาณเป็นคาบที่ต่อเนื่อง ทางเวลา ซึ่งในที่นี้สามารถที่จะพิจารณาว่า สัญญาณ x(t) เป็นสัญญาณที่มีคาบเท่ากับ mT หน่วย อย่างไรก็ตาม ค่า T ที่น้อยที่สุดที่ยังคงทำให้สมการ (2.3) เป็นจริง จะถูกเรียกว่า คาบมูลฐาน (fundamental period) ซึ่งจะเขียนแทนด้วยสัญลักษณ์ $T _ { 0 }$ (นั่นคือ เมื่อ $m = 1$ จะได้ว่า $T _ { 0 } = T )$ และส่วน กลับของคาบเวลา $T _ { 0 }$ คือ ความถี่มูลฐาน $f _ { 0 }$ (fundamental frequency) ซึ่งหาได้จากความสัมพันธ์

$$
f _ { 0 } = \frac { 1 } { T _ { 0 } }\tag{2.3}
$$

มีหน่วยเป็น เฮิรตซ์ (Hz: hertz)

![](images/chapter_2/3a11da4907a1d099cacd8ecf24cd580ed0fe1968661d8338441278f2f0986bf5.jpg)  
รูปที่ 2.2: ตัวอย่าง (a) สัญญาณเป็นคาบที่ต่อเนื่องทางเวลา และ (b) สัญญาณเป็นคาบที่ไม่ต่อเนื่อง ทางเวลา

นอกจากนี้ สัญญาณที่ไม่ต่อเนื่องทางเวลาก็สามารถเป็นสัญญาณ เป็นคาบได้ ถ้าสามารถหาคาบ เวลา N (ที่เป็นค่าบวก) ที่ทำให้

$$
x [ k ] = x [ k + N ]\tag{2.4}
$$

สำหรับทุกค่า k เมือ N เปันลขจำนวนเต็ม ดังแสดงในรูปที่ 2(b ซึ่ในทีนี้จะไ้ว่า สัญาณ [6] มีคาบเท่ากับ N, 2N, 3N, ... เช่นเดียวกัน คาบมูลฐาน $N _ { 0 }$ คือ ค่า N ที่น้อยที่สุดที่ทำให้สมการ (2.4) เป็นจริง (ในกรณีนี้คือ $N _ { 0 } = N )$

สัญญาณเป็นคาบมีคุณสมบัติที่สำคัญ คือ สัญญาณเป็นคาบเพียงหนึ่งคาบจะมีคุณลักษณะเหมือน กับสัญญาณทั้งหมดครบถ้วนทุกประการ ดังนั้น สัญญาณเป็นคาบเพียงหนึ่งคาบสามารถนำมาใช้ใน e การวิเคราะห์ข้อมูลต่างๆ ได้ครบถ้วน แทนที่จะวิเคราะห์ข้อมูลจากสัญญาณทั้งหมด สำหรับสัญญาณ อื่นๆ ที่ไม่ได้มีลักษณะเป็นสัญญาณเป็นคาบ จะ เรียกว่า สัญญาณไม่เป็นคาบ (aperiodic หรือ nonperiodic signal)

## 2.1. ประเภทของสัญญาณ

ตัวอย่างที่ 2.1 กำหนดให้ $x _ { 1 } ( t )$ และ $x _ { 2 } ( t )$ เป็นสัญญาณเป็นคาบที่มีคาบเวลาเท่ากับ $T _ { 1 }$ และ $T _ { 2 }$ ตามลำดับ จงพิจารณาว่าสัญญาณ $x ( t ) = x _ { 1 } ( t ) + x _ { 2 } ( t )$ เป็นสัญญาณเป็นคาบหรือไม่

วิธีทำ จากคุณสมบัติของสัญญาณเป็นคาบ จะได้ว่า

$$
\begin{array} { r c l } { { x _ { 1 } ( t ) } } & { { = } } & { { x _ { 1 } ( t + m T _ { 1 } ) } } \\ { { } } & { { } } & { { } } \\ { { x _ { 2 } ( t ) } } & { { = } } & { { x _ { 2 } ( t + n T _ { 2 } ) } } \end{array}
$$

เมื่อ m และ n เป็นเลขจำนวนเต็มใดๆ ถ้ากำหนดให้ $m T _ { 1 } = n T _ { 2 } = T$ แล้ว จะได้ว่า

$$
{ \begin{array} { l l l } { x ( t ) } & { = } & { x _ { 1 } ( t ) + x _ { 2 } ( t ) } \\ { \ } & { = } & { x _ { 1 } ( t + m T _ { 1 } ) + x _ { 2 } ( t + n T _ { 2 } ) } \\ { \ } & { = } & { x _ { 1 } ( t + T ) + x _ { 2 } ( t + T ) } \\ { \ } & { = } & { x ( t + T ) } \end{array} }
$$

ดังนั้น สัญญาณ x(t) จะ เป็นสัญญาณเป็นคาบ ก็ต่อเมื่ออัตราส่วน $T _ { 1 } / T _ { 2 }$ เป็นจำนวนตรรกยะ1 และ คาบเวลาของ $x ( t )$ จะ เท่ากับตัวคูณร่วมน้อย (least common multiple) ของคาบ $T _ { 1 }$ และ $T _ { 2 }$

ตัวอย่างที่ 22 จงหาคาบมูลฐาน $T _ { 0 }$ ของสัญญาณไซนูซอยด์ (sinusoidal signal) $x ( t ) = \sin ( 0 . 4 \pi t )$

วิธีทำ สัญญาณ $x ( t )$ จะมีคาบมูลฐานเท่ากับ $T _ { 0 }$ ก็ต่อเมื่อ สามารถหาค่า $T _ { 0 }$ ที่ทำให้ความสัมพันธ์ ต่อไปนี้เป็นจริง

$$
\begin{array} { r c l } { { x ( t ) } } & { { = } } & { { x ( t + T _ { 0 } ) } } \\ { { } } & { { } } & { { } } \\ { { \sin ( 0 . 4 \pi t ) } } & { { = } } & { { \sin ( 0 . 4 \pi [ t + T _ { 0 } ] ) } } \\ { { } } & { { } } & { { } } \\ { { } } & { { = } } & { { \sin ( 0 . 4 \pi t + 0 . 4 \pi T _ { 0 } ) } } \end{array}
$$

เนื่องจาก sin(θ) = sin(θ + 2nπ) เมื่อ n เป็นจำนวนเต็มใดๆ ดังนั้น คาบมูลฐาน $T _ { 0 }$ หาได้จาก
...
เมื่อ $n = 1$ นั่นคือ คาบมูลฐานของสัญญาณ A $x ( t )$ มีค่าเท่ากับ $T _ { 0 } = 5$ หน่วย

## 2.1.3 สัญญาณเชิงกำหนดและสัญญาณสุ่ม

สัญญาณเชิงกำหนด (deterministic รignal) คือ สัญญาณที่สามารถระบุได้แน่นอนว่าค่าหรือคุณสมบัติ ของสัญญาณ ณ เวลาใดๆ จะเป็นอย่างไร ดังนั้น สัญญาณประเภทนี้จึ๊งสามารถเขียนให้อยู่ในรูปของ ฟังก์ชันของเวลาได้ ในขณะที่สัญญาณสุ่ม (random sigกal) คือ สัญญาณที่ไม่สามารถระบุได้แน่นอน ว่า ค่าหรือคุณสมบัติของสัญญาณ ณ เวลาใดๆ จะเป็นอย่างไร

สำหรับในระบบสื่อสาร สัญญาณที่พบบ่อยจะมีลักษณะ เป็นสัญญาณสุ่ม โดยอาจจะอยู่ในรูปของ สัญญาณข่าวสาร ที่ต้องการหรือใช้ประโยชน์ได้ เช่น สัญญาณเสียงในระบบโทรศัพท์, สัญญาณภาพ ผ่านระบบดาวเทียม, และสัญญาณข้อมูลที่จัดเก็บในฮาร์ดดิสก์ไดรฟ์ เป็นต้น หรืออาจจะอยูในรูปของ สัญญาณรบกวน (กoiรe) ที่เกิดขึ้นในระบบซึ่งเป็นสิ่งที่ไม่ต้องการ เช่น สัญญาณรบกวนความร้อน (thermal noise) ที่เกิดจากการเคลื่อนไหวของอิเล็กตรอนในขณะใช้งานอุปกรณ์อิเล็กทรอนิกส์ ทำให้ เกิดการแผ่ความร้อนออกมาในรูปของสัญญาณรบกวน รูปที่ 2.3 แสดงตัวอย่างสัญญาณสุ่มแบบเกาส์ เซียน (Gaussian)ที่มีค่าเฉลี่ย (mean) เท่ากับค่าศูนย์ และ ค่าความแปรปรวน (variance) เท่ากับค่า หนึ่ง ซึ่งโดยทั่วไปแล้ว สัญญาณรบกวนความร้อนจะมีพฤติกรรมหรือคุณสมบัติเหมือนกับสัญญาณ สุ่มแบบเกาส์เซียน

หมายเหตุ ในหนังสือเล่มนี้ จะใช้โปรแกรม SCILAB2 [30] ในการวาดรูปกราฟของสัญญาณต่างๆ รวมทั้งผลการทดลองที่ได้จากการทำการจำลอง (รimนโลtion)ระบบ ผู้อ่านควรที่จะ พยายามทดลอง วาดรูปกราฟต่างๆ ในหนังสือเล่มนี้ เพื่อทีจะไ้ช่วยทำให้เข้าใจในบทเรียนมากยิ่งขึ้น

## 2.1. ประเภทของสัถเณาณ

![](images/chapter_2/b7442cb6f04f7d334d7b2f92b8aa83541c50387970a100e742df39b06d1a7bfa.jpg)  
รูปที่ 2.3: สัญญาณสุ่มแบบเกาส์เซียนที่มีค่าเฉลี่ยเท่ากับค่าศูนย์และค่าความแปรปรวนเท่ากับค่าหนึ่ง

## 2.1.4 สัญญาณพลังงานและสัญญาณกำลัง

ในการวิเคราะห์วงจรไฟฟ้า ถ้ากำหนดให้ $v ( t )$ คือ ค่าแรงดันไฟฟ้า มีหน่วยเป็น โวลต์ (V: volt) และ i(t) คือ กระแสไฟฟ้า มีหน่วยเป็น แอมแปร์ (A: ampere) ที่คร่อมตัวความต้านทาน R ซึ่งมีหน่วย เป็น โอห์ม (Ω: ohm) ดังนั้นจะได้ว่า กำลังขณะหนึ่ง (instantaneous power) จะ ถูกนิยามโดย

$$
p ( t ) = v ( t ) i ( t ) = \frac { 1 } { R } v ^ { 2 } ( t )\tag{2.5}
$$

เพราะฉะนั้น พลังงานรวม (total energy) ที่ใช้ไปสำหรับช่วงเวลา $t _ { 1 } \leq t \leq t _ { 2 }$ คือ

$$
\int _ { t _ { 1 } } ^ { t _ { 2 } } p ( t ) d t = \int _ { t _ { 1 } } ^ { t _ { 2 } } \frac { 1 } { R } v ^ { 2 } ( t )\tag{2.6}
$$

และกำลังเฉลี่ย (average power) ที่ใช้ไปสำหรับช่วงเวลา $t _ { 1 } \leq t \leq t _ { 2 }$ คือ

$$
{ \frac { 1 } { t _ { 2 } - t _ { 1 } } } \int _ { t _ { 1 } } ^ { t _ { 2 } } p ( t ) d t = { \frac { 1 } { t _ { 2 } - t _ { 1 } } } \int _ { t _ { 1 } } ^ { t _ { 2 } } { \frac { 1 } { R } } v ^ { 2 } ( t )\tag{2.7}
$$

จากนิยามของพลังงาน (energy) และกำลัง (poพer) ที่ใช้ในวงจรไฟฟ้า พลังงานรวมของสัญญาณ ที่ต่อเนืองทางเวลา $x ( t )$ ที่ใช้ไปสำหรับช่วงเวลา $t _ { 1 } \leq t \leq t _ { 2 }$ สามารถที่จะนิยามได้ ดังนี้

$$
\int _ { t _ { 1 } } ^ { t _ { 2 } } | x ( t ) | ^ { 2 } d t\tag{2.8}
$$

โดยที่ $| x |$ คือ ขนาด (magnitนde) ของ x และค่ากำลังเฉลี่ยทางเวลา (time-averaged power) ของ $x ( t )$ คือ

$$
{ \frac { 1 } { t _ { 2 } - t _ { 1 } } } \int _ { t _ { 1 } } ^ { t _ { 2 } } | x ( t ) | ^ { 2 } d t\tag{2.9}
$$

เช่นเดียวกัน พลังงานรวมของสัญญาณที่ไม่ต่อเนื่องทางเวลา $x [ k ]$ ที่ใช้ไปสำหรับช่วงเวลา $k _ { 1 } \leq k \leq$ $k _ { 2 }$ สามารถที่จะนิยามได้ ดังนี้

$$
\sum _ { k = k _ { 1 } } ^ { k _ { 2 } } \left| x [ k ] \right| ^ { 2 }\tag{2.10}
$$

e $x [ k ]$ คือ และค่ากาลังเฉลียทางเวลาของ

$$
{ \frac { 1 } { k _ { 2 } - k _ { 1 } + 1 } } \sum _ { k = k _ { 1 } } ^ { k _ { 2 } } | x [ k ] | ^ { 2 }\tag{2.11}
$$

อย่างไรก็ตาม ในทางปฏิบัติมักจะสนใจพลังงานของสัญญาณสำหรับช่วงเวลาอนันต์ นั่นคือ เมื่อ $- \infty \leq t \leq \infty \ u _ { t } \mathfrak { s } ^ { \flat } \ - \infty \leq k \leq \infty$ ดังนั้นในกรณีนี้จะได้ว่า พลังงานรวมของสัญญาณที่ต่อเนื่อง ซ ทางเวลา คือ

$$
E = \operatorname* { l i m } _ { T \to \infty } \int _ { - T } ^ { T } | x ( t ) | ^ { 2 } d t = \int _ { - \infty } ^ { \infty } | x ( t ) | ^ { 2 } d t\tag{2.12}
$$

และพลังงานรวมของสัญญาณที่ไม่ต่อเนืองทางเวลา คือ 2ท 4

$$
E = \operatorname* { l i m } _ { N \to \infty } \sum _ { k = - N } ^ { N } | x [ k ] | ^ { 2 } = \sum _ { k = - \infty } ^ { \infty } | x [ k ] | ^ { 2 }\tag{2.13}
$$

ในทำนองเดียวกันจะได้ว่า กำลังเฉลี่ยทางเวลาของสัญญาณที่ต่อเนื่องทางเวลา $x ( t )$ ที่ใช้ไปสำหรับ ช่วงเวลาอนันต์ คือ

$$
P = \operatorname* { l i m } _ { T  \infty } \frac { 1 } { 2 T } \int _ { - T } ^ { T } | x ( t ) | ^ { 2 } d t\tag{2.14}
$$

## 2.1. ประเภทของสัญญาณ

และกำลังเฉลี่ยทางเวลาของสัญญาณที่ไม่ต่อเนื่องทางเวลา $x [ k ]$ ที่ใช้ไปสำหรับช่วงเวลาอนันต์ คือ

$$
P = \operatorname* { l i m } _ { N  \infty } \frac { 1 } { 2 N + 1 } \sum _ { k = - N } ^ { N } | x [ k ] | ^ { 2 }\tag{2.15}
$$

สัญญาณใดๆ จะ ถูกเรียกว่า สัญญาณกำลัง (power signal) ก็ต่อเมื่อ ค่ากำลังของสัญญาณมีค่า จำกัด กล่าวคือ $0 < P < \infty$ ดังแสดงในตัวอย่างต่อไปนี้

ตัวอย่างที่ 2.3 จงหากำลังของสัญญาณ $x ( t ) = A \cos ( \omega t + \phi )$ เมื่อ $\omega = 2 \pi f$ และ $\phi$ เป็นค่าคงตัว (constant) ของมุมเฟส

วิธีทำ เนื่องจาก สัญญาณ $x ( t )$ เป็นสัญญาณเป็นคาบที่มีคาบเวลาเท่ากับ $T _ { 0 }$ ดังนั้น กำลังของ สัญญาณสามารถหาได้จากสมการ (2.14) นั้นคือ

$$
\begin{array} { r l } { \rho - \underset { \bar { T } ^ { * } \to \infty } { \operatorname* { l i m } } \frac { 1 } { 2 T } \int _ { \eta } ^ { \eta } | z | \| ^ { 2 } d t \ : - } & { \frac { 1 } { T _ { 0 } } \int _ { 0 } ^ { \bar { T } _ { \eta } } | x ( t ) | ^ { 2 } d t } \\ { = } & { \frac { 1 } { T _ { 0 } } \int _ { 0 } ^ { \bar { T } _ { \eta } } | 4 \cos ( \omega t + \phi ) | ^ { 2 } d t } \\ & { - \frac { 1 } { T _ { 0 } } \lambda ^ { 2 } \int _ { 0 } ^ { \bar { T } _ { \eta } } \cos ^ { 2 } ( \phi t ) \ \varphi \mathrm { d } t } \\ { = } & { \frac { 1 } { T _ { 0 } } 4 ^ { 2 } \int _ { 0 } ^ { \bar { T } _ { \eta } } \left\{ \frac { 1 + \cos ( 2 \phi t + 2 \phi ) } { 2 } \right\} \mathrm { d } t } \\ { = } & { \frac { 1 } { 2 T _ { 0 } } 4 ^ { 2 } \left\{ \int _ { 0 } ^ { \bar { T } _ { \eta } } \left( 1 \right) d t \ : \int _ { 0 } ^ { \bar { T } _ { \eta } } \cos ( 2 \phi t + 2 \phi ) \right\} \mathrm { d } t } \\ { = } & { \frac { 1 } { 2 T _ { 0 } } 4 ^ { 2 } \{ \bar { T } _ { 0 } + 0 \} } \\ { = } & { \frac { 4 ^ { 2 } } { 2 T _ { 0 } } 4 ^ { 2 } \left( \bar { T } _ { 0 } + 0 \right) } \\ { = } & { \frac { 4 ^ { 2 } } { \alpha ^ { 2 } } } \end{array}
$$

เนืองจาก $\begin{array} { r } { \int _ { 0 } ^ { T _ { 0 } } \cos ( 2 \omega t + 2 \phi ) \} d t = 0 } \end{array}$

จากตัวอย่างที่ 2.3 จะพบว่า กำลังของสัญญาณเป็นคาบสามารถคำนวณหาได้โดยการใช้สัญญาณ

เพียงคาบเดียว นั้นคือ กำลังของสัญญาณเป็นคาบ $x ( t )$ ที่มีคาบเวลาเท่ากับ $T _ { 0 }$ หาได้จาก

$$
P = \frac { 1 } { T _ { 0 } } \int _ { 0 } ^ { T _ { 0 } } | x ( t ) | ^ { 2 } d t\tag{2.16}
$$

แต่พลังงานของสัญญาณเป็นคาบจะมีค่าเป็นค่าอนันต์ (ดูจากสมการ (2.12)) เพราะฉะนั้นโดยทั่วไป สัญญาณ เป็นคาบจะ ถูก พิจารณาว่าเป็นสัญญาณกำลัง ในขณะที่สำหรับสัญญาณไม่เป็นคาบ เมื่อ ทำ การหาค่าเฉลี่ยของสัญญาณบนช่วงเวลาอนันต์จะได้ผลลัพธ์เป็นค่าศูนย์ ดังนั้น สัญญาณไม่เป็นคาบ ไม่สามารถหากำลังของสัญญาณได้ จึงต้องใช้นิยามคำว่า "พลังงาน" เข้ามาช่วย กล่าวคือ สัญญาณ ใดๆ จะ ถูกเรียกว่า สัญญาณพลังงาน (energy รignal) ก็ต่อเมื่อ ค่าพลังงานของสัญญาณมีค่าจำกัด นั่นคือ $0 < E < \infty$

ตัวอย่างที่ 2.4 จงพิจารณาสัญญาณที่ต่อเนื่องทางเวลาในรูปที่ 2.4 ว่าเป็นสัญญาณกำลังหรือสัญญาณ พลังงาน เมื่อ c เป็นค่าคงตัวใดๆ
...
(b)  
รูปที่ 2.4: ตัวอย่าง (a) สัญญาณไม่เป็นคาบที่ต่อเนื่องทางเวลา และ (b) สัญญาณเป็นคาบที่ต่อเนื่อง ทางเวลา

วิธีทำ

## 2.1. ประเภทของสัญญาณ

ก) จากรูป 2.4(a) กำลังของสัญญาณ $x ( t )$ มีค่าเท่ากับ

$$
\begin{array} { l l l } { { P } } & { { = } } & { { \displaystyle \operatorname* { l i m } _ { T \to \infty } \frac { 1 } { 2 T } \int _ { - T } ^ { T } | x ( t ) | ^ { 2 } d t } } \\ { { } } & { { = } } & { { \displaystyle \operatorname* { l i m } _ { T \to \infty } \frac { 1 } { 2 T } \int _ { - c } ^ { c } A ^ { 2 } d t } } \\ { { } } & { { = } } & { { \displaystyle \operatorname* { l i m } _ { T \to \infty } \frac { 1 } { 2 T } ( 2 c A ^ { 2 } ) } } \\ { { } } & { { = } } & { { 0 } } \end{array}
$$

และพลังงานของสัญญาณ $x ( t )$ มีค่าเท่ากับ

$$
\begin{array} { l l l } { { { \cal E } } } & { { = } } & { { \displaystyle \int _ { - \infty } ^ { \infty } | x ( t ) | ^ { 2 } d t } } \\ { { } } & { { = } } & { { \displaystyle \int _ { - c } ^ { c } A ^ { 2 } d t } } \\ { { } } & { { = } } & { { 2 c A ^ { 2 } } } \end{array}
$$

ซ ดังนั้น สัญญาณ $x ( t )$ ในรูป 2.4(a) จึงเป็นสัญญาณพลังงาน เนื่องจาก $0 < E < \infty$

ข)จากรูป 2.4(b) กำลังของสัญญาณ $y ( t )$ มีค่าเท่ากับ

$$
\begin{array} { l l l } { { P } } & { { = } } & { { \displaystyle \operatorname* { l i m } _ { T \to \infty } \frac { 1 } { 2 T } \int _ { - T } ^ { T } | y ( t ) | ^ { 2 } d t } } \\ { { } } & { { = } } & { { \displaystyle \frac { 1 } { 4 c } \int _ { - c } ^ { 3 c } | y ( t ) | ^ { 2 } d t } } \\ { { } } & { { = } } & { { \displaystyle \frac { 1 } { 4 c } \left\{ \int _ { - c } ^ { c } ( A ) ^ { 2 } d t + \int _ { c } ^ { 3 c } ( - A ) ^ { 2 } d t \right\} } } \\ { { } } & { { = } } & { { \displaystyle \frac { 1 } { 4 c } \left\{ 2 c A ^ { 2 } + 2 c A ^ { 2 } \right\} } } \\ { { } } & { { = } } & { { \displaystyle A ^ { 2 } } } \end{array}
$$

และพลังงานของสัญญาณ $x ( t )$ มีค่าเท่ากับ

$$
\begin{array} { r c l } { E } & { = } & { \displaystyle \int _ { - \infty } ^ { \infty } | y ( t ) | ^ { 2 } d t } \\ & { = } & { \infty } \end{array}
$$