...
通常，在通信系统中传输的各种信息具有随机信号的行为。因此，在分析通信系统时，必须首先将随机信号转换为确定性信号的形式，以便于分析。本章介绍了用于将随机信号转换为确定性形式的各种数学工具，如均值、自相关函数、自协方差函数和功率谱密度等。此外，还讨论了遍历过程的特性以及随机过程与线性时不变 (LTI) 系统之间的关系。这些知识对于分析后续章节中将要讨论的硬盘驱动器信号处理系统至关重要。

## 4.5 本章习题

1. 给定随机变量 $X$ 和常数 $a$，证明 $\mathrm{Var}(aX) = a^2 \mathrm{Var}(X)$。

2. 证明定义在方程 (4.23) 中的相关系数满足 $-1 \le \rho_{XY} \le 1$。

3. 给定随机变量 $X$ 和 $N$ 互不相关，且均值为零，即 $E[X] = E[N] = 0$，方差分别为 $\sigma_X^2$ 和 $\sigma_N^2$。若定义随机变量 $Y = X + N$，请计算：
   - a) 随机变量 $Y$ 的方差。
   - b) 随机变量 $X$ 和 $Y$ 之间的相关系数 $\rho_{XY}$，并用参数 $\sigma_X$ 和 $\sigma_N$ 表示。
   - c) 若 $\rho_{XY} = 0.99$，计算所需信号平均功率与噪声平均功率之比（信噪比 SNR: signal-to-noise ratio）$\mathrm{SNR} = \sigma_X^2 / \sigma_N^2$ 的分贝 (dB) 值。

4. 证明方程 (4.28) 中的高斯概率密度函数的全区域面积等于 1。（提示：从概率密度函数积分的平方函数入手考虑）。

5. 给定随机变量 $Y = aX + b$，其中 $a$ 和 $b$ 为常数，且随机变量 $X$ 具有均值为 $\mu$、方差为 $\sigma^2$ 的高斯概率密度函数，即 $X \sim \mathcal{N}(\mu, \sigma^2)$。证明 $Y \sim \mathcal{N}(a\mu + b, a^2\sigma^2)$。

6. 给定随机变量 $X \sim \mathcal{N}(\mu, \sigma^2)$，证明 $E[X] = \mu$ 且 $E[(X-\mu)^2] = \sigma^2$。

7. 给定随机变量 $X \sim \mathcal{N}(0, \sigma_X^2)$ 和 $N \sim \mathcal{N}(a, \sigma_N^2)$，若随机变量 $Y = X + bN$：
   - a) 若随机变量 $X$ 和 $N$ 互不相关，请求出随机变量 $Y$ 的方差，并用参数 $a, b, \sigma_X$ 和 $\sigma_N$ 表示。
   - b) 求随机变量 $X$ 和 $Y$ 之间的相关系数 $\rho_{XY}$，并用参数 $a, b, \sigma_X$ 和 $\sigma_N$ 表示。

8. 给定 $X = \cos(\Theta)$ 和 $Y = \sin(\Theta)$，其中 $\Theta$ 是在区间 $[0, 2\pi]$ 上服从均匀分布 (uniform distribution) 的随机变量。证明：
   - a) $X$ 和 $Y$ 互不相关。
   - b) $X$ 和 $Y$ 不是独立的。

9. 给定 $X(t)$ 为宽平稳 (WSS) 随机过程。证明对于所有 $\tau$，均有 $|R_X(\tau)| \le R_X(0)$。

10. 给定 $X(t)$ 为宽平稳 (WSS) 随机过程，其均值为 $m_X$，自相关函数为 $R_X(\tau)$。若定义随机过程 $Y(t) = X(t) + 2X(t-T)$，请计算 $m_Y$ 和 $R_Y(\tau)$。

11. 给定一个 LTI 系统，其冲激响应为 $h(t) = \mathrm{sinc}(2t)$。若输入信号 $N(t)$ 为宽平稳 (WSS) 随机过程，且均值 $m_N = 0$，自相关函数 $R_N(\tau) = \frac{N_0}{2}\delta(\tau)$。请计算输出信号 $W(t) = N(t) * h(t)$ 的均值和自相关函数，其中符号 $*$ 表示卷积运算。
